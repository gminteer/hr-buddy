    ...
    employee: [
      ...
      },
      {
        ...
        choices: async (answers) => {
          const department_id = await db.getDeptIdFromRoleId(answers.role);
          const sameDept = await db.getEmployeesByDeptId(department_id);
          return sameDept.map((row) => ({name: `${row.first_name} ${row.last_name}`, value: row.id}));
        },
      },
    ],
  };
